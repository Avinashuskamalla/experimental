...
import com.spanner.demo.exceptions.RestApiException;
import com.spanner.demo.repository.EmployeeEducationRepository;
import com.spanner.demo.repository.EmployeeRepository;
import com.spanner.demo.response.EmployeeResponse;
import com.spanner.demo.util.AppUtil;
import com.spanner.demo.util.FileStorage;
...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public Mono<?> getAllemployees() {
	 ...
	 return Mono.just(EmployeeResponse.builder()		  
		  .statusCode(HttpStatus.OK.value()) .message(empData) .build());
	...
	public Mono<?> searchEmployee(String empId) {
		...
		return Mono.just(EmployeeResponse.builder()
				...
				.statusCode(HttpStatus.OK.value())
				.message(modelMapper.map(e.get(), EmployeeDto.class))
				.build());
	}
		...
		try {
			...
			return employeeData.log().map(edu->{
				...
				return Mono.just(EmployeeResponse.builder()
						...
						.statusCode(HttpStatus.OK.value())
						.message("Employee is successfully Updaed...!")
						.build());
	...
	public Mono<?> uploadCertificate(String empId, MultipartFile filePart) {
		...
		if (!StringUtils.isEmpty(emp.get().getEmpCertificate()))
		{
		 ...
		 return storage.uploadFiles(filePart,imageType).flatMap(urlInfo->{
			 ...
			 return Mono.just(EmployeeResponse.builder()					 
						.statusCode(HttpStatus.OK.value())
						.message(storage.getBucketAddress().concat(filePart.getOriginalFilename()))
						.build());
		 });
	...
	public Mono<?> deleteCertificate(String id) {
		...
		emp.orElseThrow(() -> new RestApiException("Employee Not found",HttpStatus.BAD_REQUEST));
		 ...
		 return Mono.just(EmployeeResponse.builder()
					...
					.statusCode(HttpStatus.OK.value())
					.message(storage.deleteFile(emp.get().getEmpCertificate()))
					.build());
	}
